...
const defaultOutPath = "reports/teaching-archive-material-draft-intent.current.json";
const sourceFiles = {
  gateway: "contracts/agent/controlled-write-intent-gateway.example.json",
  openapi: "contracts/openapi/teaching-archive.archive-material-draft-intents.path.yaml",
  domain: "services/teaching-archive-gateway/internal/domain/teaching_archive_material_draft_intent.go",
  usecase: "services/teaching-archive-gateway/internal/usecase/submit_teaching_archive_material_draft_intent.go",
  commandlog: "services/teaching-archive-gateway/internal/adapter/commandlog/archive_material_draft_intent.go",
  http: "services/teaching-archive-gateway/internal/adapter/httpapi/server_archive_material_draft_intent.go",
...
export function auditTeachingArchiveMaterialDraftIntent(inputs, options = {}) {
  const findings = [];
  const gateway = parseJSON(inputs.gateway, {});

  addFinding(findings, {
    id: "contract.gateway_allowlists_archive_material_draft",
    passed: (gateway.acceptedIntents ?? []).some((intent) =>
      intent.intentId === "draft_archive_material" &&
      intent.commandPort?.portName === "TeachingDraftCommandPort" &&
      intent.commandPort?.operation === "submitArchiveMaterialDraftIntent" &&
    ...
    expected: "draft_archive_material -> TeachingDraftCommandPort.submitArchiveMaterialDraftIntent, review-only",
    remediation: "Keep the Agent contract aligned with the Teaching archive material draft runtime port.",
  });
  ...
  addFinding(findings, {
    ...
    remediation: "The endpoint must submit review-only command intent, not create final archive material.",
  });
  ...
  addFinding(findings, {
    ...
    expected: "principal, target owner, draft artifact, guardrail, approval, rollback, audit, and idempotency required",
    remediation: "Do not accept Agent archive material draft intent without full review evidence.",
  });
  ...
  addFinding(findings, {
    ...
    remediation: "Archive material draft intent must not create archive items, OCR jobs, RAG chunks, or AI grading rows.",
  });
  ...
  addFinding(findings, {
    ...
    actual: summarizePresence(inputs.http, ["http.StatusAccepted", "http.StatusCreated", "contentRef"]),
    expected: "route submits review-only command intent and never returns final archive material contentRef",
    remediation: "HTTP must stay a command-intent ingress, not a final archive item creation endpoint.",
  });
  ...
  return {
    ...
    nextAction: readiness === "READY"
      ? "Use this as the second controlled Agent write-intent runtime slice; continue module-by-module refactor without reopening broad performance testing."
      : "Fix Teaching archive material draft intent runtime boundaries before adding any execution path.",
  };
}

export function formatTeachingArchiveMaterialDraftIntentAudit(report) {
  const lines = [
    `Teaching archive material draft intent runtime: ${report.readiness}`,
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    const report = auditTeachingArchiveMaterialDraftIntent(loadCurrentInputs(root));
    writeReport(root, args.outPath, report);
    console.log(formatTeachingArchiveMaterialDraftIntentAudit(report));
