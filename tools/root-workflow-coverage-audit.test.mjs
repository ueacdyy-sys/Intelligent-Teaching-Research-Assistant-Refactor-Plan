...
describe("root workflow coverage audit", () => {
  ...
  it("fails when workflow/plugin draft intent evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginDraftIntent];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginDraftIntent").passed, false);
  });
  it("fails when workflow/plugin sandbox result evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginSandboxResult];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginSandboxResult").passed, false);
  });
  it("fails when workflow/plugin human approval evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginHumanApproval];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginHumanApproval").passed, false);
  });
  it("fails when workflow/plugin registry admission runtime evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginRegistryAdmissionRuntime];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginRegistryAdmissionRuntime").passed, false);
  });
  it("fails when workflow/plugin execution isolation evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginExecutionIsolation];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginExecutionIsolation").passed, false);
  });
  it("fails when workflow/plugin publication disabled evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginPublicationDisabled];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginPublicationDisabled").passed, false);
  });
  it("fails when workflow/plugin management disabled view evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginManagementDisabledView];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginManagementDisabledView").passed, false);
  });
  it("fails when workflow/plugin management audit detail evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginManagementAuditDetail];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginManagementAuditDetail").passed, false);
  });
  it("fails when workflow/plugin management read-only list evidence is missing", () => {
    const inputs = currentInputs();
    delete inputs.reports[sourceReports.workflowPluginManagementReadonlyList];
    const report = auditRootWorkflowCoverage(inputs);
    const workflow = report.workflows.find((candidate) => candidate.id === "workflow_plugin_self_evolution");
    assert.equal(report.readiness, "NEEDS_REMEDIATION");
    assert.equal(workflow.coverageStatus, "NEEDS_EVIDENCE");
    assert.equal(workflow.reportResults.find((result) => result.key === "workflowPluginManagementReadonlyList").passed, false);
  });
  ...
  it("fails when Student App AI Tutor request, generation claim, input envelope, teacher review, or draft visibility evidence is missing or slow", () => {
    ...
    assert.equal(workflow.runtimeEvidenceResults[9].passed, false);
    ...
    assert.equal(workflow.runtimeEvidenceResults[11].passed, false);
    ...
    assert.equal(workflow.runtimeEvidenceResults[12].passed, false);
    ...
    assert.equal(workflow.runtimeEvidenceResults[13].passed, false);
    ...
    assert.equal(workflow.runtimeEvidenceResults[14].passed, false);
  });
...
function runtimeNames(workflow) {
  return workflow.runtimeEvidenceResults.map((result) => result.name);
}
