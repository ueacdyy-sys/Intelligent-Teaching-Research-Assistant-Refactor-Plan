...
the Root SLO promotion review still reads
...
That is now the wrong evidence boundary for the production 10k claim. The root
review must consume the current target-bearing production10k sustained report
before it can fairly decide whether the claim is supported or blocked.
...
- Keep the review conservative: the current default report passes throughput
  but still blocks promotion while max P99 is above the `300ms` root target.
...
- `node --test tools/root-slo-promotion-review-audit.test.mjs` proves the
  current review uses the production10k report, reports `22836.8 RPS`, and
  blocks only on `production_target.max_p99_ms=310.78`.
...
- `npm run audit:root-slo-promotion-review` regenerates
  `reports/root-slo-promotion-review.current.json` with the corrected blocker.
- `npm run audit:system-capacity-claim` reflects the updated Root SLO blocker
