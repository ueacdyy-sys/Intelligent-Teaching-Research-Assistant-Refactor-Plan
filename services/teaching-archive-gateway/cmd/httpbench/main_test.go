...
func TestBuildPhaseReport(t *testing.T) {
	...
	if phase.Name != "createArchiveItem" || phase.Operations != 2 || phase.Errors != 0 || phase.RPS != 10 {
...
func TestBuildPhaseReportIncludesClientServerGap(t *testing.T) {
	phase := buildPhaseReport(
		...
		[]time.Duration{20 * time.Millisecond, 30 * time.Millisecond},
		[]map[string]time.Duration{
			{"handler": 12 * time.Millisecond, "app": 10 * time.Millisecond},
			{"handler": 18 * time.Millisecond, "app": 14 * time.Millisecond},
		},
		nil,
		0,
		100*time.Millisecond,
...
func TestBuildPhaseReportIncludesClientTraceBreakdown(t *testing.T) {
	phase := buildPhaseReport(
		...
		[]map[string]time.Duration{
			{"handler": 12 * time.Millisecond, "app": 10 * time.Millisecond},
			{"handler": 16 * time.Millisecond, "app": 14 * time.Millisecond},
		},
		[]map[string]time.Duration{
			{
...
func TestDoJSONCapturesClientTraceTimings(t *testing.T) {
	...
	result, err := doJSON(
		...
		server.Client(),
		http.MethodPost,
		server.URL+"/v1/teaching/archive-items",
		"local-key",
...
func TestCreateArchiveItemSendsTeachingPrincipalAndCapturesID(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		if request.Header.Get("X-Agent-Api-Key") != "local-key" {
			http.Error(response, "missing local key", http.StatusUnauthorized)
			return
		}
		principal := decodePrincipalHeader(t, request.Header.Get("X-Principal-Context"))
		if principal["role"] != "TEACHER" {
			t.Fatalf("principal role = %v want TEACHER", principal["role"])
		}
		var body map[string]any
		if err := json.NewDecoder(request.Body).Decode(&body); err != nil {
			t.Fatalf("decode request body: %v", err)
		}
		if body["ownerType"] != "TEACHING" || body["materialType"] != "QUIZ" {
			t.Fatalf("body = %#v", body)
		}
		if !strings.Contains(body["contentRef"].(string), "go-httpbench") {
			t.Fatalf("contentRef = %v want go-httpbench marker", body["contentRef"])
		}
		response.Header().Set("Server-Timing", "handler;dur=2, app;dur=1, db.insert;dur=0.8")
		response.WriteHeader(http.StatusCreated)
		_, _ = response.Write([]byte(`{"id":"tarch_go_1"}`))
	}))
	t.Cleanup(server.Close)
	...
	if result.archiveItemID != "tarch_go_1" {
		t.Fatalf("archiveItemID = %q want tarch_go_1", result.archiveItemID)
	}
	assertDuration(t, result.serverTimings, "handler", 2*time.Millisecond)
	if result.clientTraceTimings["client.round_trip"] <= 0 {
		t.Fatalf("client.round_trip = %s want positive", result.clientTraceTimings["client.round_trip"])
	}
}
...
func decodePrincipalHeader(t *testing.T, value string) map[string]any {
	t.Helper()
	data, err := base64.RawURLEncoding.DecodeString(value)
	if err != nil {
		t.Fatalf("decode principal header: %v", err)
	}
