...
func parseStudentAppQuestionBankDraftSummaryPath(path string) bool {
	return path == "/v1/student-app/question-bank-drafts/summary"
}
...
func parseStudentAppQuestionBankDraftAnswerSubmissionAIFeedbackPath(path string) (string, bool) {
	const prefix = "/v1/student-app/question-bank-draft-answer-submissions/"
	const suffix = "/ai-feedback"
	if !strings.HasPrefix(path, prefix) || !strings.HasSuffix(path, suffix) {
		return "", false
	}
	submissionID := strings.TrimSuffix(strings.TrimPrefix(path, prefix), suffix)
...
func parseStudentAppQuestionBankDraftAnswerSubmissionAIFeedbackLearningActionsPath(path string) (string, bool) {
	const prefix = "/v1/student-app/question-bank-draft-answer-submissions/"
	const suffix = "/ai-feedback/learning-actions"
	if !strings.HasPrefix(path, prefix) || !strings.HasSuffix(path, suffix) {
		return "", false
	}
	submissionID := strings.TrimSuffix(strings.TrimPrefix(path, prefix), suffix)
