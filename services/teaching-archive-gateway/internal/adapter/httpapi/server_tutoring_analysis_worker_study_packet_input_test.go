...
func TestReadAITutorWorkerStudyPacketInputReturnsResultArchiveSafeWorkerPackage(t *testing.T) {
	handler := newTestHandlerWithAITutorWorkerResultArchiveInput()
	request := httptest.NewRequest(
		http.MethodPost,
		"/v1/teaching/tutoring-analysis-requests/tutor_req_http_worker_result_archive_input/ai-tutor-study-packet-input",
		bytes.NewBufferString(`{"workerId":" worker_student_tutor_01 "}`),
	)
	request.Header.Set("X-Agent-Api-Key", "ueacd")
	setPrincipalHeader(t, request, servicePrincipal())
	...
	for _, fragment := range [][]byte{
		[]byte(`"requestId":"tutor_req_http_worker_result_archive_input"`),
		...
		[]byte(`"resultArchiveSourceItemId":"tarch_source_student_homework_001"`),
		[]byte(`"renderFormat":"SAFE_TEXT_BLOCKS"`),
		[]byte(`"blockType":"SUMMARY"`),
		[]byte(`"blockType":"GUIDANCE_SECTION"`),
		[]byte(`"sourceBlockRefs":["block_section_001"]`),
	} {
		if !bytes.Contains(response.Body.Bytes(), fragment) {
			t.Fatalf("body missing %s in %s", fragment, response.Body.String())
		}
...
func newTestHandlerWithAITutorWorkerResultArchiveInput() http.Handler {
	now := time.Date(2026, 6, 8, 10, 0, 0, 0, time.UTC)
	tutoringRequest := tutoringAnalysisRequest(
		"tutor_req_http_worker_result_archive_input",
		"tarch_student_ai_tutor_result_001",
		"student_001",
		now.Add(-10*time.Minute),
	)
	tutoringRequest.AnalysisGoal = "continue from reviewed AI Tutor guidance"
	tutoringRequest.QuestionBankIntent = domain.QuestionBankIntentGeneratePersonalizedCheck
	tutoringRequest.Status = domain.TutoringAnalysisStatusInProgress
	tutoringRequest.LearningActionSource = domain.StudentAppAITutorLearningActionSourceResultArchive
	tutoringRequest.FollowUpDepth = 1
